...
const render = async () => {
    ...
    moreDataSource = moreData = (await categoryModel.cateProdList({guidIds:datasource[0].guidIds,pageNo:pageNo})).data;
...
const clickEvent = () => {
    $('.content-left ul li').on("tap", async function(){
        ...
        moreData = (await categoryModel.cateProdList({guidIds:data.guidIds,pageNo:pageNo})).data;
        showRightContent(data);
        scroll();
    });
...
const showRightContent =  function(data) {
    ...
    let html = template({moreDataSource:moreData==""?moreDataSource:moreData,data});
...
const scroll =  () => {
    ...
    cateScroll.on('scrollEnd',async function(){
        ...
        if(maxY - y > -400 && maxY - y <=0){
            let result =  (await categoryModel.cateProdList({guidIds:data.guidIds,pageNo:++pageNo})).data;
            moreData = [
                ...moreData,
                ...result
            ]
